...
.content{
  ...
  margin-left:240px;
  padding-top:80px;   /* ⭐ HARUS sama dengan tinggi header */
}
...
.kantin-card.offline:hover{
  transform: none;
  box-shadow: none;
}

.kantin-card.offline::after{
...
<div class="app">
  ...
  <main class="content">
    ...
    <section class="kantin">
      @foreach($kantin as $data)
        ...
        @if($data->isOnline())
          <a href="{{ route('pilih_kantin', ['id' => $data->id]) }}"
            class="kantin-card">
            KANTIN {{ $data->no_kantin }}
          </a>
        @else
          <div class="kantin-card offline">
            KANTIN {{ $data->no_kantin }}
          </div>
        @endif
      ...
      <section class="kategori">
        <a href="{{ route('index') }}"><button>Semua</button></a>
        <a href="{{ route('index.makanan') }}"><button>Makanan</button></a>
        <a href="{{ route('index.minuman') }}"><button class="active">Minuman</button></a>
      </section>
  ...
  <div id="qty-box-{{ $data->id }}"
      ...
      <!-- ✅ TOMBOL SIMPAN (INI YANG PENTING) -->
      <button type="submit"
              class="btn-tambah"
